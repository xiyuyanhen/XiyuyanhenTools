...
public class MIDI {
    ...
    public init() {
        AudioKit_Log("Initializing MIDI", log: OSLog.midi)
        ...
        if client == 0 {
            ...
            if result != noErr {
                AudioKit_Log("Error creating MIDI client: \(result)", log: OSLog.midi, type: .error)
            }
        }
    }
    ...
    func startReceivingSysEx(with midiBytes: [MIDIByte]) {
        AudioKit_Log("Starting to receive SysEx", log: OSLog.midi)
        isReceivingSysEx = true
        incomingSysEx = midiBytes
    }
    func stopReceivingSysEx() {
        AudioKit_Log("Done receiving SysEx", log: OSLog.midi)
