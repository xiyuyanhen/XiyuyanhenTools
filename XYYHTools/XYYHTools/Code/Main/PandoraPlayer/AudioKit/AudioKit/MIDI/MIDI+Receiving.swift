...
extension MIDI {
    ...
    private func processUMPSysExMessage(with bytes: [UInt8]) -> [UInt8]? {
        ...
        guard (umpSysExStatus != nil) else {
            AudioKit_Log("UMP SYSEX - Got unsupported UMPSysEx7bitStatus", log: OSLog.midi)
            return validBytes
        }
        ...
        case .CompleteMessage:
            AudioKit_Log("UMP SYSEX - Got complete SysEx message in one UMP packet", log: OSLog.midi)
        ...
        case .Start:
            AudioKit_Log("UMP SYSEX - Start receiving UMP SysEx messages", log: OSLog.midi)
            ...
            return nil
        case .Continue:
            AudioKit_Log("UMP SYSEX - Continue receiving UMP SysEx messages", log: OSLog.midi)
            
            incomigUMPSysExMessage.append(contentsOf: validBytes)
            // Full message not ready, nothing to return
            return nil
        case .End:
            AudioKit_Log("UMP SYSEX - End of UMP SysEx messages", log: OSLog.midi)
    ...
    private func processUMPMessages(_ midiEventPacket: MIDIEventList.UnsafeSequence.Element) -> [MIDIEvent] {
        ...
        while (wordIndex < midiEventPacket.pointee.wordCount) {
            ...
            guard (umpMessageType != nil) else {
                AudioKit_Log("Got invalid UMP Message Type, skipping rest of the packet", log: OSLog.midi)
                return midiEvents
            }
            ...
            case .Reserved32bit_1, .Reserved32bit_2:
                AudioKit_Log("Got unsupported 32 bit UMP message of type: \(String(describing: umpMessageType))", log: OSLog.midi)
                //print(wordCollection[wordIndex])
                wordIndex += 1
                break
            ...
            case .MIDI2ChannelVoice64bit, .Reserved64bit_3, .Reserved64bit_4, .Reserved64bit_5:
                AudioKit_Log("Got unsupported 64 bit UMP message of type: \(String(describing: umpMessageType))", log: OSLog.midi)
                //print(Array(wordCollection[wordIndex...wordIndex + 1]))
                wordIndex += 2
                break
            case .Reserved96bit_6, .Reserved96bit_7:
                AudioKit_Log("Got unsupported 96 bit UMP message of type: \(String(describing: umpMessageType))", log: OSLog.midi)
                //print(Array(wordCollection[wordIndex...wordIndex + 2]))
                wordIndex += 3
                break
            case .Data128bit, .Reserved128bit_8, .Reserved128bit_9, .Reserved128bit_10:
                AudioKit_Log("Got unsupported 128 bit UMP message of type \(String(describing: umpMessageType))", log: OSLog.midi)
                //print(Array(wordCollection[wordIndex...wordIndex + 3]))
                wordIndex += 4
                break
            default:
                // We should not get there, because of the guard at the top
                AudioKit_Log("Received undefined UMP Message type", log: OSLog.midi)
            ...
            if inputUID == 0 || inputUID == uid {
                ...
                if var port = inputPorts[inputUID] {
                    ...
                    if #available(iOS 14.0, *) {
                        ...
                        inputPortCreationResult = MIDIInputPortCreateWithProtocol(client, inputPortName, ._1_0, &port) { eventPacketList, _ in
                            ...
                            guard (eventPacketList.pointee.protocol == ._1_0) else {
                                AudioKit_Log("Got unsupported MIDI 2.0 MIDIEventList, skipping", log: OSLog.midi)
                                return
                            }
                    ...
                    if inputPortCreationResult != noErr {
                        AudioKit_Log("Error creating MIDI Input Port: \(inputPortCreationResult)")
                    }
    ...
    public func closeInput(uid inputUID: MIDIUniqueID) {
        guard let name = inputName(for: inputUID) else {
            AudioKit_Log("Trying to close midi input \(inputUID), but no name was found", log: OSLog.midi)
            return
        }
        AudioKit_Log("Closing MIDI Input '\(name)'", log: OSLog.midi)
        var result = noErr
            ...
            if inputUID == 0 || uid == inputUID {
                if let port = inputPorts[uid], let endpoint = endpoints[uid] {
                    ...
                    if result == noErr {
                        ...
                        AudioKit_Log("Disconnected \(name) and removed it from endpoints and input ports", log: OSLog.midi)
                    } else {
                        AudioKit_Log("Error disconnecting MIDI port: \(result)", log: OSLog.midi, type: .error)
                    }
                    result = MIDIPortDispose(port)
                    if result == noErr {
                        AudioKit_Log("Disposed \(name)", log: OSLog.midi)
                    } else {
                        AudioKit_Log("Error displosing  MIDI port: \(result)", log: OSLog.midi, type: .error)
                    }
                }
            }
        }
    ...
    public func closeAllInputs() {
        AudioKit_Log("Closing All Inputs", log: OSLog.midi)
        ...
        for listener in listeners {
            ...
            if let type = event.status?.type {
                guard let eventChannel = event.channel else {
                    AudioKit_Log("No channel detected in handleMIDIMessage", log: OSLog.midi)
                    continue
                }
                switch type {
                ...
                AudioKit_Log("No usable status detected in handleMIDIMessage", log: OSLog.midi)
            }
        }
    }
