...
extension MIDI {
    ...
    public func openOutput(uid outputUid: MIDIUniqueID) {
        if outputPort == 0 {
            guard let tempPort = MIDIOutputPort(client: client, name: outputPortName) else {
                AudioKit_Log("Unable to create MIDIOutputPort", log: OSLog.midi, type: .error)
                return
            }
            outputPort = tempPort
        }
    ...
    public func closeOutput(uid outputUid: MIDIUniqueID) {
        ...
        AudioKit_Log("Closing MIDI Output '\(String(describing: name))'", log: OSLog.midi)
        var result = noErr
        if endpoints[outputUid] != nil {
            endpoints.removeValue(forKey: outputUid)
            AudioKit_Log("Disconnected \(name) and removed it from endpoints", log: OSLog.midi)
            if endpoints.isEmpty {
                // if there are no more endpoints, dispose of midi output port
                result = MIDIPortDispose(outputPort)
                if result == noErr {
                    AudioKit_Log("Disposed MIDI Output port", log: OSLog.midi)
                } else {
                    AudioKit_Log("Error disposing  MIDI Output port: \(result)", log: OSLog.midi, type: .error)
                }
                outputPort = 0
            }
        }
    ...
    public func sendMessage(_ data: [MIDIByte], offset: MIDITimeStamp = 0) {
        ...
        if bufferSize > 65_536 {
            AudioKit_Log("error sending midi : data array is too large, requires a buffer larger than 65536",
                  log: OSLog.midi,
                  type: .error)
            return
        }
        ...
        buffer.withUnsafeMutableBytes { (ptr: UnsafeMutableRawBufferPointer) -> Void in
            if let packetListPointer = ptr.bindMemory(to: MIDIPacketList.self).baseAddress {
                ...
                if nextPacket == nil {
                    AudioKit_Log("error sending midi: Failed to add packet to packet list.", log: OSLog.midi, type: .error)
                    return
                }
                ...
                for endpoint in endpoints.values {
                    ...
                    if result != noErr {
                        AudioKit_Log("error sending midi: \(result)", log: OSLog.midi, type: .error)
                    }
                }
