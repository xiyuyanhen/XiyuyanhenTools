...
public class AudioEngine {
    ...
    public var input: InputNode? {
        if #available(macOS 10.14, *) {
            guard Bundle.main.object(forInfoDictionaryKey: "NSMicrophoneUsageDescription") != nil else {
                AudioKit_Log("To use the microphone, you must include the NSMicrophoneUsageDescription in your Info.plist", type: .error)
                return nil
            }
        }
        ...
        didSet {
            ...
            if let node = output {
                ...
                if let currentSampleRate = mainMixerNode?.avAudioUnitOrNode.outputFormat(forBus: 0).sampleRate,
                    ...
                    AudioKit_Log("Sample Rate has changed, creating new mainMixerNode at", Settings.sampleRate)
                    removeEngineMixer()
                }
    ...
    public func start() throws {
        if output == nil {
            AudioKit_Log("🛑 Error: Attempt to start engine with no output.")
            return
        }
        try avEngine.start()
    }
    ...
    public func startTest(totalDuration duration: Double) -> AVAudioPCMBuffer {
        ...
        do {
            ...
            AudioKit_Log("🛑 Start Test Error: \(err)")
        }
    ...
    public func render(duration: Double) -> AVAudioPCMBuffer {
        ...
        do {
            ...
            AudioKit_Log("🛑 Could not render offline \(err)")
        }
        return buffer
    }
    ...
    public var inputDevice: Device? {
        if let portDescription = AVAudioSession.sharedInstance().preferredInput {
            ...
            if inputDevices.isNotEmptyOfAudioKit {
