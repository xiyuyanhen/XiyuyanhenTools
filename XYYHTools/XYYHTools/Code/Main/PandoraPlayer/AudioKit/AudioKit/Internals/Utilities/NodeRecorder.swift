...
open class NodeRecorder: NSObject {
    ...
    open var audioFile: AVAudioFile? {
        do {
            ...
            AudioKit_Log("Error, Cannot create internal audio file for reading: \(error.localizedDescription)")
            return nil
        }
    }
    ...
    public init(node: Node,
        ...
        guard let file = file else {
            AudioKit_Log("Error, no file to write to")
            return
        }
        ...
        do {
            ...
            AudioKit_Log("Error: cannot write to", file.url)
            throw error
        }
    ...
    public static func createTempFile() -> AVAudioFile? {
        ...
        AudioKit_Log("Creating temp file at", url)
    ...
    public static func removeTempFiles() {
        for url in NodeRecorder.tmpFiles {
            ...
            AudioKit_Log("𝗫 Deleted tmp file at", url)
        }
    ...
    public func record() throws {
        if isRecording == true {
            AudioKit_Log("Warning: already recording")
            return
        }
        ...
        AudioKit_Log("⏺ Recording using format", internalAudioFile?.processingFormat.debugDescription)
    ...
    private func process(buffer: AVAudioPCMBuffer, time: AVAudioTime) {
        ...
        do {
            ...
            AudioKit_Log("Write failed: error -> \(error.localizedDescription)")
        }
    }
    ...
    public func stop() {
        if isRecording == false {
            AudioKit_Log("Warning: Cannot stop recording, already stopped")
            return
        }
    ...
    public func reset() throws {
        ...
        do {
            ...
            AudioKit_Log("Error: Can't delete" + (audioFile?.url.lastPathComponent ?? "nil") + error.localizedDescription)
        }

        // Creates a blank new file
        do {
            self.internalAudioFile = try AVAudioFile(forWriting: url, settings: settings)
            AudioKit_Log("File has been cleared")
        } catch let error as NSError {
            AudioKit_Log("Error: Can't record to" + url.lastPathComponent)
            throw error
        }
    }
}
