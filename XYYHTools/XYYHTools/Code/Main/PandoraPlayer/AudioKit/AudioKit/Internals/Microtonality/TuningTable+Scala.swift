...
extension TuningTable {
    ...
    public func scalaFile(_ filePath: String) -> Int? {
        guard
            ...
            AudioKit_Log("can't read filePath: \(filePath)")
            return nil
        }
    ...
    fileprivate func stringTrimmedForLeadingAndTrailingWhiteSpacesFromString(_ inputString: String?) -> String? {
        ...
        do {
            ...
            AudioKit_Log("ERROR: create regex: \(error)")
            return nil
        }
    ...
    public func frequencies(fromScalaString rawStr: String?) -> [Frequency]? {
        ...
        do {
            ...
            AudioKit_Log("ERROR: cannot parse scala file: \(error)")
        ...
        for rawLineStr in values {
            ...
            if parsedFirstNonCommentLine && !parsedAllFrequencies {
                if let newFrequencyCount = Int(lineStr) {
                    ...
                    if frequencyCount == 0 || frequencyCount > 127 {
                        ...
                        AudioKit_Log("ERROR: number of notes in scala file: \(frequencyCount)")
                        parsedScala = false
                        break
                    } else {
            ...
            if actualFrequencyCount > frequencyCount {
                AudioKit_Log("actual frequency cont: \(actualFrequencyCount) > frequency count: \(frequencyCount)")
            }
            ...
            if NSEqualRanges(rangeOfFirstMatch, NSRange(location: NSNotFound, length: 0)) == false {
                ...
                if substringForFirstMatch.range(of: ".").length != 0 {
                    ...
                    if substringForFirstMatch.range(of: "/").length != 0 {
                        if substringForFirstMatch.range(of: "-").length != 0 {
                            AudioKit_Log("ERROR: invalid ratio: \(substringForFirstMatch)")
                            parsedScala = false
                            break
                        }
                ...
                AudioKit_Log("ERROR: error parsing: \(lineStr)")
                continue
            }
        }

        if !parsedScala {
            AudioKit_Log("FATAL ERROR: cannot parse Scala file")
            return nil
        }

        AudioKit_Log("frequencies: \(scalaFrequencies)")
        return scalaFrequencies
    }
}
