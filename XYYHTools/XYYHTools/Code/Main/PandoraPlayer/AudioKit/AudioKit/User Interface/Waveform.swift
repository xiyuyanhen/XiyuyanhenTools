...
public class Waveform: CALayer {
    ...
    private func updateReverse() {
        guard !plots.isEmpty else {
            AudioKit_Log("Waveform isn't ready to be reversed yet. No data.", type: .error)
            return
        }
        let direction: CGFloat = isReversed ? -1.0 : 1.0

        // AudioKit_Log("Current Direction:", reverseDirection, "proposed direction:", direction)
        ...
        // AudioKit_Log("REVERSING:", reverseDirection)
    }
    ...
    private func fillPlots(with data: FloatChannelData, completionHandler: (() -> Void)? = nil) {
        ...
        if !plots.isEmpty {
            if let left = data.first {
                // AudioKit_Log("** Updating table data", left.count, "points")
                plots[0].table = left
        ...
        // create the plots
        // AudioKit_Log("** Creating plots... channels:", data.count)
    ...
    private func createPlot(data: [Float], color: CGColor) -> WaveformLayer {
        // AudioKit_Log(data.count, "plotSize", plotSize)
    ...
    public func updateLayer() {
        guard plots.isNotEmptyOfAudioKit else {
            AudioKit_Log("Plots are empty... nothing to layout.", type: .error)
            return
        }
