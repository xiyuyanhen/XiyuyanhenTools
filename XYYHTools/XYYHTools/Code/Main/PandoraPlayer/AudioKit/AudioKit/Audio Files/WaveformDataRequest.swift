...
public class WaveformDataRequest {
    ...
    deinit {
        AudioKit_Log("* { WaveformDataRequest }")
        audioFile = nil
    }
    ...
    public func getData(with samplesPerPixel: Int) -> FloatChannelData? {
        ...
        for i in 0 ..< samplesPerPixel {
            if abortGetWaveformData {
                ...
                AudioKit_Log("* Aborting waveform data get *", type: .error)
                return nil
            }
            ...
            do {
                ...
                AudioKit_Log("Error: Couldn't read into buffer. \(err)", log: .fileHandling, type: .error)
                return nil
            }
