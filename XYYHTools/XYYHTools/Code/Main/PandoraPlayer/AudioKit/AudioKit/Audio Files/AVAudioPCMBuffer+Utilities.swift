...
extension AVAudioPCMBuffer {
    ...
    @discardableResult public func copy(from buffer: AVAudioPCMBuffer,
        ...
        if remainingCapacity == 0 {
            AudioKit_Log("AVAudioBuffer copy(from) - no capacity!")
            return 0
        }

        if format != buffer.format {
            AudioKit_Log("AVAudioBuffer copy(from) - formats must match!")
            return 0
        }
        ...
        if totalFrames <= 0 {
            AudioKit_Log("AVAudioBuffer copy(from) - No frames to copy!")
            return 0
        }
    ...
    public func extract(from startTime: TimeInterval,
        ...
        guard frameCapacity > 0 else {
            AudioKit_Log("startSample must be before endSample", type: .error)
            return nil
        }
        ...
        guard editedBuffer.copy(from: self, readOffset: startSample, frames: frameCapacity) > 0 else {
            AudioKit_Log("Failed to write to edited buffer", type: .error)
            return nil
        }
