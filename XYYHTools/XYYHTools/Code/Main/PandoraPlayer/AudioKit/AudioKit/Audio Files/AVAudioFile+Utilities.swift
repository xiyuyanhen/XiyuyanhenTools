...
extension AVAudioFile {
    ...
    public func toAVAudioPCMBuffer() -> AVAudioPCMBuffer? {
        ...
        do {
            ...
            AudioKit_Log("Created buffer with format", processingFormat)

        } catch let error as NSError {
            AudioKit_Log("Cannot read into buffer " + error.localizedDescription, log: OSLog.fileHandling, type: .error)
        }
    ...
    @discardableResult public func extract(to outputURL: URL,
        ...
        guard let inputBuffer = toAVAudioPCMBuffer() else {
            AudioKit_Log("Error reading into input buffer", type: .error)
            return nil
        }

        guard var editedBuffer = inputBuffer.extract(from: startTime, to: endTime) else {
            AudioKit_Log("Failed to create edited buffer", type: .error)
            return nil
        }
        ...
        guard let outputFile = try? AVAudioFile(url: outputURL, fromBuffer: editedBuffer) else {
            AudioKit_Log("Failed to write new file at", outputURL, type: .error)
            return nil
        }
        return outputFile
    }
    ...
    public func extract(to url: URL,
        ...
        converter.start { error in
            ...
            if let error = error {
                AudioKit_Log("Done, error", error, type: .error)
            }
            ...
            do {
                ...
                AudioKit_Log("Unable to remove temp file at", tempFile, type: .error)
            }
        }
    }
}
