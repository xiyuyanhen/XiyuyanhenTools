...
extension FormatConverter {
    ...
    internal func convertAsset(completionHandler: FormatConverterCallback? = nil) {
        ...
        if FileManager.default.fileExists(atPath: outputURL.path) {
            if options.eraseFile {
                AudioKit_Log("Warning: removing existing file at", outputURL.path)
        ...
        case "wav":
            ...
            formatKey = kAudioFormatLinearPCM
        default:
            AudioKit_Log("Unsupported output format: \(outputFormat)")
            return
        }
        ...
        if !writer.startWriting() {
            AudioKit_Log("Failed to start writing. Error:", writer.error?.localizedDescription)
        ...
        if !reader.startReading() {
            AudioKit_Log("Failed to start reading. Error:", reader.error?.localizedDescription)
        ...
        writerInput.requestMediaDataWhenReady(on: queue, using: {
            ...
            while writerInput.isReadyForMoreMediaData, processing {
                if reader.status == .reading,
                    ...
                    case .failed:
                        AudioKit_Log("Conversion failed with error", reader.error)
                        writer.cancelWriting()
                        completionHandler?(reader.error)
                    case .cancelled:
                        AudioKit_Log("Conversion cancelled")
                    ...
                    case .completed:
                        ...
                        writer.finishWriting {
                            ...
                            case .failed:
                                completionHandler?(writer.error)
                            default:
                                // AudioKit_Log("Conversion complete")
                                completionHandler?(nil)
                            }
                        }
                    default:
    ...
    internal func convertCompressed(completionHandler: FormatConverterCallback? = nil) {
        ...
        AudioKit_Log("Converting to AVAssetExportPresetAppleM4A with default settings.")
    ...
    internal func convertToPCM(completionHandler: FormatConverterCallback? = nil) {
        ...
        AudioKit_Log("convertToPCM() to \(outputURL)")
        ...
        guard inputFormat != outputFormat ||
            ...
            AudioKit_Log("No conversion is needed, formats are the same. Copying to", outputURL)
            // just copy it?
            do {
