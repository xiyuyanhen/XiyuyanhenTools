...
public class PresetBuilder {
    ...
    public static func createAUPreset(dict: [NSMutableDictionary],
        ...
        for i in 0 ..< dict.count {
            ...
            for loadedSoundDict in loadSoundsArr {
                guard let alreadyLoadedSound: String = loadedSoundDict.object(forKey: filenameKey) as? String,
                    ...
                    AudioKit_Log("Unable to load new sound in PresetBuilder")
                    return
                }
                if alreadyLoadedSound == newLoadingSound {
                    alreadyLoaded = true
                    guard let temp = loadedSoundDict.object(forKey: "sampleNum") as? Int else {
                        AudioKit_Log("No sampleNum provided in PresetBuilder")
                        return
                    }
                    sampleNum = temp
                }
            ...
            if !alreadyLoaded { // if this is a new sound, then add it to samplefile xml
                ...
                guard let samplePath = sound.object(forKey: "filename") as? String else {
                    AudioKit_Log("No filename provided in PresetBuilder")
                    return
                }
        ...
        // write to file
        do {
            // AudioKit_Log("Writing to \(path)")
            try str.write(toFile: path, atomically: false, encoding: String.Encoding.utf8)
        } catch let error as NSError {
            AudioKit_Log("Could not write to \(path)")
            AudioKit_Log(error.localizedDescription)
        }
    }
