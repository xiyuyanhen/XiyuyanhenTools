...
public class AudioPlayer: Node {
    ...
    override func makeAVConnections() {
        guard let engine = mixerNode.engine else {
            AudioKit_Log("Engine is nil", type: .error)
            return
        }
    ...
    public func play(
        ...
        guard playerNode.engine != nil else {
            AudioKit_Log("🛑 Error: AudioPlayer must be attached before playback.")
            return
        }
...
extension AudioPlayer {
    ...
    public func schedule(
        ...
        if isBuffered, let buffer = buffer {
            ...
            AudioKit_Log("The player needs a file or a valid buffer to schedule", type: .error)
            scheduleTime = nil
        }
    }
    ...
    public func scheduleBuffer(url: URL,
        ...
        guard let buffer = try? AVAudioPCMBuffer(url: url) else {
            AudioKit_Log("Failed to create buffer", type: .error)
            return
        }
