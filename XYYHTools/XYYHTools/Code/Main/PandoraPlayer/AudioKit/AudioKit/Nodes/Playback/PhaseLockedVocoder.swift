...
public class PhaseLockedVocoder: Node, AudioUnitContainer, Tappable, Toggleable {
    ...
    internal func loadFile(_ avAudioFile: AVAudioFile) {
        Exit: do {
            ...
            if err != 0 { AudioKit_Log("ExtAudioFileOpenURL FAILED, Error = \(err)"); break Exit }
            ...
            err = ExtAudioFileGetProperty(externalAudioFileRef,
                                          ...
                                          &theFileFormat)
            if err != 0 {
                AudioKit_Log("ExtAudioFileGetProperty(kExtAudioFileProperty_FileDataFormat) FAILED, Error = \(err)")
                break Exit
            }
            if theFileFormat.mChannelsPerFrame > 2 {
                AudioKit_Log("Unsupported Format, channel count is greater than stereo")
                break Exit
            }
            ...
            err = ExtAudioFileSetProperty(externalAudioFileRef,
                                          ...
                                          &theOutputFormat)
            if err != 0 {
                AudioKit_Log("ExtAudioFileSetProperty(kExtAudioFileProperty_ClientDataFormat) FAILED, Error = \(err)")
                break Exit
            }
            ...
            if err != 0 {
                AudioKit_Log("ExtAudioFileGetProperty(kExtAudioFileProperty_FileLengthFrames) FAILED, Error = \(err)")
                break Exit
            }
            ...
            if theData != nil {
                ...
                if err == noErr {
                    ...
                    AudioKit_Log("Error = \(err)"); break Exit
                }
            }
        }
    }
