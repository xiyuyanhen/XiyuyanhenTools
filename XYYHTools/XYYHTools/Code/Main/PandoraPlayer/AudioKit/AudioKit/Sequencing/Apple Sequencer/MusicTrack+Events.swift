...
extension MusicTrackManager {
    ...
    public func debug() {
        ...
        for event in events {
            ...
            case kMusicEventType_MIDINoteMessage:
                ...
                AudioKit_Log("MIDI Note @:\(event.time) note:\(note) velocity:\(velocity) duration:\(dur) channel:\(channel)")
            case kMusicEventType_Meta:
                ...
                guard let midiData = data?.pointee.data,
                    ...
                    AudioKit_Log("Problem with raw midi meta message")
                    return
                }
                AudioKit_Log("MIDI Meta @ \(event.time) - size: \(length) - type: \(type) - data: \(midiData)")
            case kMusicEventType_MIDIChannelMessage:
                ...
                guard let data1 = data?.pointee.data1,
                    ...
                    AudioKit_Log("Problem with raw midi channel message")
                    return
                }
                if let statusType = MIDIStatus(byte: statusData)?.type {
                    switch statusType {
                    case .programChange:
                        AudioKit_Log("MIDI Program Change @ \(event.time) - program: \(data1) - channel: \(statusData.lowBit)")
                    default:
                        AudioKit_Log("MIDI Channel Message @\(event.time) data1:\(data1) data2:\(data2) status:\(statusType)")
                    }
                }
            default:
                AudioKit_Log("MIDI Event @ \(event.time)")
            }
        }
    }
    ...
    private func getRawEventData(ofType type: MusicEventType? = nil) -> [AppleMIDIEvent]? {
        ...
        guard let track = internalMusicTrack else {
            AudioKit_Log("debug failed - track doesn't exist")
            return events
        }
