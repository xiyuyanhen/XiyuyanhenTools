...
open class MusicTrackManager {
    ...
    open var isNotEmptyOfAudioKit: Bool {
        return !isEmpty
    }
    ...
    public init(name: String = "Unnamed") {
        ...
        if let track = internalMusicTrack {
            ...
            if result != 0 {
                AudioKit_Log("Unable to name Track")
            }
        }
    }
    ...
    public func setLength(_ duration: Duration) {
        ...
        guard let newSequence = tempSequence else {
            AudioKit_Log("Unable to create temp sequence in setLength")
            return
        }
        ...
        guard let newTrack = tempTrack,
            ...
            AudioKit_Log("internalMusicTrack does not exist")
            return
        }
        ...
        if isNotEmptyOfAudioKit {
            ...
            guard let iterator = tempIterator else {
                AudioKit_Log("Unable to create iterator in setLength")
                return
            }
            ...
            while hasNextEvent.boolValue {
                ...
                if eventType == kMusicEventType_MIDINoteMessage {
                    ...
                    guard let channel = data?.pointee.channel,
                        ...
                        AudioKit_Log("Problem with raw midi note message")
                        return
                    }
    ...
    private func clearHelper(_ targetEventType: UInt32, from functionName: String) {
        ...
        NewMusicEventIterator(track, &tempIterator)
        guard let iterator = tempIterator else {
            AudioKit_Log("Unable to create iterator in \(functionName)")
            return
        }
        var eventTime = MusicTimeStamp(0)
        var eventType = MusicEventType()
    ...
    public func clearNote(_ note: MIDINoteNumber) {
        ...
        NewMusicEventIterator(track, &tempIterator)
        guard let iterator = tempIterator else {
            AudioKit_Log("Unable to create iterator in clearNote")
            return
        }
        var eventTime = MusicTimeStamp(0)
        var eventType = MusicEventType()
    ...
    open var isEmpty: Bool {
        ...
        NewMusicEventIterator(track, &tempIterator)
        guard let iterator = tempIterator else {
            AudioKit_Log("Unable to create iterator in isEmpty")
            return true
        }
        var outBool = true
        var eventTime = MusicTimeStamp(0)
    ...
    public func addSysEx(_ data: [MIDIByte], position: Duration) {
        ...
        if result != 0 {
            AudioKit_Log("Unable to insert raw midi data")
        }
    }
    ...
    class func iterateMusicTrack(_ track: MusicTrack,
        ...
        NewMusicEventIterator(track, &tempIterator)
        guard let iterator = tempIterator else {
            AudioKit_Log("Unable to create iterator")
            return
        }
        var eventTime = MusicTimeStamp(0)
        var eventType = MusicEventType()
