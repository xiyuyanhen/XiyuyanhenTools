...
open class Sequencer {
    ...
    public required init(targetNodes: [Node]? = nil) {
        if let targetNodes = targetNodes {
            ...
            AudioKit_Log("no nodes connected to sequencer at init - be sure to connect some via addTrack")
        }
    }
    ...
    public func load(midiFile: MIDIFile) {
        ...
        if midiTracks.count > tracks.count {
            AudioKit_Log("Error: Track count and file track count do not match ",
                  "dropped \(midiTracks.count - tracks.count) tracks")
        }
        if tracks.count > midiTracks.count {
            AudioKit_Log("Error: Track count less than file track count, ignoring \(tracks.count - midiTracks.count) nodes")
        }
    ...
    public func add(noteNumber: MIDINoteNumber,
        ...
        guard tracks.count > trackIndex, trackIndex >= 0 else {
            AudioKit_Log("Track index \(trackIndex) out of range (sequencer has \(tracks.count) tracks)")
            return
        }
