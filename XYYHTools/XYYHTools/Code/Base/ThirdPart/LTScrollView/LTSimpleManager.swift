...
//    @objc public init(viewControllers: [UIViewController], titles: [String], currentViewController:UIViewController, layout: LTLayout, titleView: LTPageTitleView? = nil) {
//
//        UIScrollView.initializeOnce()
//
//        self.viewControllers = viewControllers
...
//    }
 ...
 extension LTSimpleManager {
    ...
    @discardableResult public func removeByDeinit() -> [UIViewController] {
        ...
        self.titleView.mainScrollView = nil
        self.titleView.glt_createViewControllerHandle = nil
        self.titleView.glt_didSelectTitleViewHandle = nil
        ...
        self.contentTableView?.removeSubviews()
        ...
        for viewController in viewControllers {
            ...
            for childrenVC in viewController.children {
 ...
 extension LTSimpleManager {
    private func setupRefreshData()  {
        DispatchQueue.main.after(time: DispatchTime.now() + .milliseconds(1)) {
 ...
 extension LTSimpleManager: UITableViewDelegate {
    ...
    public func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        return // fixBug 多个ViewControllers之间切换出现内容ScrollView固定
        
        guard scrollView == self.tableView,
            let contentTableView = self.contentTableView else { return }
        
        let contentTableViewOffsetY = contentTableView.contentOffset.y

        let svOffsetY = scrollView.contentOffset.y
        
        let cha = self.kHeaderHeight - self.hoverY

        if (0 < contentTableViewOffsetY) || (cha < svOffsetY) {

            scrollView.contentOffset = CGPoint(x: 0.0, y: cha)
        }
        ...
        if svOffsetY < cha {
            ...
            for viewController in viewControllers {
                ...
                guard let glt_scrollView = viewController.glt_scrollView,
                    glt_scrollView != scrollView else { continue }

                glt_scrollView.contentOffset = .zero
            }
        }
    }
 ...
 extension LTSimpleManager {
    ...
    private func setupHeaderView(headerView: UIView) {
        ...
        if self.layout.isHovered == false {
            //此处需要延时一下才有效
            DispatchQueue.main.after(time: DispatchTime.now() + .microseconds(100)) {
                //设置titleView的y值
