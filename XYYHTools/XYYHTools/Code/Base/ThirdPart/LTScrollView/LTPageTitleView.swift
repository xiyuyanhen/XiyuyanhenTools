...
@objc public class LTPageTitleView: UIView {
    ...
    public var layout: LTLayout = LTLayout()
    ...
    lazy var sliderScrollView: UIScrollView = {
        
        let sliderScrollView = UIScrollView(frame: self.sliderScrollViewFrame)
    ...
    private var sliderScrollViewFrame: CGRect {
        
        return CGRect(x: self.layout.titleContentEdgeInsets.left,
                      y: self.layout.titleContentEdgeInsets.top,
                      width: bounds.width - self.layout.titleContentEdgeInsets.left - self.layout.titleContentEdgeInsets.right,
                      height: bounds.height - self.layout.titleContentEdgeInsets.top - self.layout.titleContentEdgeInsets.bottom)
    }
    
    private lazy var pageBottomLineView: UIView = {
        let pageBottomLineView = UIView(frame: self.pageBottomLineViewFrame)
        pageBottomLineView.backgroundColor = layout.pageBottomLineColor
        return pageBottomLineView
    }()
    
    private var pageBottomLineViewFrame: CGRect {
        
        return CGRect(x: 0, y: bounds.height - layout.pageBottomLineHeight, width: bounds.width, height: layout.pageBottomLineHeight)
    }
    
    private lazy var sliderLineView: UIView = {
        let sliderLineView = UIView(frame: self.sliderLineViewFrame)
        sliderLineView.backgroundColor = layout.bottomLineColor
        return sliderLineView
    }()
    
    private var sliderLineViewFrame: CGRect {
        
        return CGRect(x: layout.lMargin, y: bounds.height - layout.bottomLineHeight - layout.pageBottomLineHeight, width: 0, height: layout.bottomLineHeight)
    }
...
extension LTPageTitleView {
    ...
    func setupSubViews(selectedIndex: Int = 0) {
        
        self.updateFrame()
        
        self.addSubview(self.sliderScrollView)
        
        self.sliderScrollView.addSubview(self.sliderLineView)
        
        self.addSubview(self.pageBottomLineView)
        
        self.setupButtonsLayout(selectedIndex: selectedIndex)
    }
    
    private func updateFrame() {
        
        if self.sliderScrollView.frame != self.sliderScrollViewFrame {
            
            self.sliderScrollView.frame = self.sliderScrollViewFrame
        }
        
        if self.sliderLineView.frame != self.sliderLineViewFrame {
            
            self.sliderLineView.frame = self.sliderLineViewFrame
        }
        
        if self.pageBottomLineView.frame != self.pageBottomLineViewFrame {
            
            self.pageBottomLineView.frame = self.pageBottomLineViewFrame
        }
    }
}
...
extension LTPageTitleView {
    ...
    private func setupButtonsLayout(selectedIndex: Int) {
        
        self.glt_textWidths.removeAll()
        self.glt_lineWidths.removeAll()
        
        for button in self.self.glt_buttons {
        ...
        for (_, text) in titles.enumerated() {
            if layout.isAverage {
                let textAverageW = (bounds.width - layout.lMargin - layout.rMargin - layout.titleMargin * CGFloat(titles.count - 1)) / CGFloat(titles.count)
        ...
        for index in 0..<titles.count {
            ...
            let color = (index == selectedIndex ? layout.titleSelectColor : layout.titleColor)
        ...
        if layout.sliderWidth == glt_sliderDefaultWidth {
            if layout.isAverage {
                ...
                sliderLineView.frame.origin.x = (firstTextWidth - firstLineWidth) * 0.5 + layout.lMargin
            }else {
            ...
            sliderLineView.frame.origin.x = ((firstTextWidth + layout.lMargin + layout.rMargin) - layout.sliderWidth) * 0.5
        }
        ...
        let sliderContenSizeW = upX - layout.titleMargin + layout.lMargin + layout.rMargin
...
extension LTPageTitleView {
    ...
    private func setupLineViewX(offsetX: CGFloat) -> Bool {
        ...
        if offsetX + scrollW >= scrollContenSizeW {
            if layout.sliderWidth == glt_sliderDefaultWidth {
                ...
                sliderLineView.frame.origin.x = layout.rMargin + adjustX
            }else {
        ...
        if layout.sliderWidth == glt_sliderDefaultWidth {
            ...
            /*
             * 原理：按钮的最左边X（因为有lMargin，这里必须减掉） 以及 按钮的相对右边X（注意不是最右边，因为每个按钮的X都有一个lMargin， 所以相对右边则有两个才能保证按钮的位置，这个和titleMargin无关）
             */
            let maxNextLeft = nextButton.frame.origin.x - layout.lMargin
            let maxNextRight = maxNextLeft + layout.lMargin * 2.0 + nextButton.frame.size.width
            ...
            let maxRight = maxLeft + layout.lMargin * 2.0 + currentButton.frame.size.width
    ...
    private func setupSliderLineViewWidth(currentButton: UIButton)  {
        let maxLeft = currentButton.frame.origin.x - layout.lMargin
        let maxRight = maxLeft + layout.lMargin * 2 + currentButton.frame.size.width
        let originX = (maxRight - maxLeft - layout.sliderWidth) * 0.5  + maxLeft
